...
import { customerLoginSchema, loginCustomer, toCustomerDto } from "@srpanel/core"
...
export const POST = route<{ slug: string }>(async (req, ctx) => {
	...
	const body = await parseBody(req, customerLoginSchema)
	const h = await headers()
	const { token, customer } = await loginCustomer(store.settings, body, { ip: await clientIp(), userAgent: h.get("user-agent") })
